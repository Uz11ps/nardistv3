...
# Проверяем что находится перед @fallback
echo ""
echo "Строки перед @fallback:"
sed -n "$((FALLBACK_LINE-3)),$((FALLBACK_LINE-1))p" "$CONFIG_FILE"

# Закрывающая скобка location / должна быть на строке перед @fallback
CLOSING_BRACE=$((FALLBACK_LINE - 1))
...
echo ""
echo "Показываем строки 45-50:"
sed -n '45,50p' "$CONFIG_FILE"
