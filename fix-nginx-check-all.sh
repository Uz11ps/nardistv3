...
# Скрипт для проверки всего файла после изменений
...
cp "$TMP_FILE" /tmp/nardist_check.conf

echo ""
echo "Проверка созданного файла:"
echo "Все location блоки:"
grep -n "^[[:space:]]*location" /tmp/nardist_check.conf

echo ""
echo "Все location / (включая вложенные):"
grep -n "location /" /tmp/nardist_check.conf

echo ""
echo "Строки 40-70:"
sed -n '40,70p' /tmp/nardist_check.conf

echo ""
echo "Проверка синтаксиса временного файла:"
if nginx -t -c /tmp/nardist_check.conf 2>&1; then
    ...
    nginx -t -c /tmp/nardist_check.conf 2>&1
fi
