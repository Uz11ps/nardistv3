...
export default function BackgammonBoard({
  ...
  const MOBILE_CONFIG = {
    ...
    diceP2X: 0.09,
    diceP2Y: 0.38, 
    checkerTopOffset: -255, 
    checkerBottomOffset: 255,
    // Legacy text offsets
    textTopOffset: -15,
    textBottomOffset: 15,
    // New parameters for advanced highlight and text control
  ...
  const loadDebugConfig = useCallback(() => {
    try {
      const saved = localStorage.getItem('backgammon-debug-config-v6')
      if (saved) {
  ...
  useEffect(() => {
    if (debugMode) {
      try {
        localStorage.setItem('backgammon-debug-config-v6', JSON.stringify(debugConfig))
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const hasSavedConfig = localStorage.getItem('backgammon-debug-config-v6')
