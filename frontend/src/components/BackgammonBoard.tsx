    ...
    const fetchPossibleMoves = async () => {
      ...
      try {
        ...
        if (gameMode === 'long' && flatMoves.length > 0) {
          ...
          const BOARD_SIZE = 24
          const WHITE_HOME_START = 18 // Points 1-6 (indices 18-23)
          const BLACK_HOME_START = 6 // Points 13-18 (indices 6-11)
          const BLACK_HEAD = 12
          
          // Проверка: есть ли у противника хотя бы одна шашка в доме
          let opponentHasCheckerInHome = false
          if (activePlayer === 0) {
            // Проверяем черные шашки в черном доме (indices 6-11)
            ...
            // Проверяем белые шашки в белом доме (indices 18-23)
