...
export default function Game() {
  ...
  const checkAutoConfirmRef = useRef<((moves: Array<{ from: number; to: number; die: number; steps?: any[] }>) => void) | null>(null)
  ...
  const setupSocketHandlers = (socket: any) => {
    ...
    socket.on('timer_update', (data: any) => {
      if (data.gameId === gameId) {
        ...
        // Используем timeLimitSeconds из настроек игры, если доступно
        ...
        if (data.currentPlayer === 0) {
          ...
          setPlayer2Timer(timeLimitSeconds) // Соперник имеет полное время на ход
          player2TimerRef.current = timeLimitSeconds
        } else {
          // Ход игрока 2
          setPlayer2Timer(moveTimeRemaining)
          player2TimerRef.current = moveTimeRemaining
          setPlayer1Timer(timeLimitSeconds) // Соперник имеет полное время на ход
          player1TimerRef.current = timeLimitSeconds
        }
      }
    })
  ...
  const handleMove = async (from: number, to: number, die: number, steps?: any[]) => {
    ...
    setPendingMoves(prev => {
      const newMoves = [...prev, { from, to, die, steps }]
      
      // Если подтверждение не требуется, проверяем авто-подтверждение сразу после добавления хода
      if (!requireConfirmMove && 
        ...
        setTimeout(() => {
          checkAutoConfirmRef.current?.(newMoves)
        }, 100)
      }
  ...
  const checkAutoConfirm = useCallback(async (moves: Array<{ from: number; to: number; die: number; steps?: any[] }>) => {
    ...
    // ВАЖНО: Если таймер уже установлен, не переустанавливаем его
    if (autoConfirmTimeoutRef.current) {
      return
    }
    ...
    if (allDiceUsed) {
      ...
      autoConfirmTimeoutRef.current = window.setTimeout(() => {
        ...
        autoConfirmTimeoutRef.current = null // Очищаем ref перед вызовом
        handleConfirm()
      }, 3000)
      return
    }
  ...
  useEffect(() => {
    checkAutoConfirmRef.current = checkAutoConfirm
  }, [checkAutoConfirm])

  // Автоматическое подтверждение хода, если requireConfirmMove = false
  // Проверка через useEffect при изменении pendingMoves
  useEffect(() => {
    ...
    if (!requireConfirmMove && 
      ...
      // Проверяем только если таймер еще не установлен
      if (!autoConfirmTimeoutRef.current) {
    ...
    return () => {
      // НЕ очищаем таймер при изменении зависимостей - пусть он сработает
    }
  }, [pendingMoves.length, requireConfirmMove, isMyTurn, gameStatus, isSandbox, isProcessingConfirm, checkAutoConfirm])
            ...
            <div className={`game-player ${!isPlayer1 ? 'game-player-me' : ''}`}>
              ...
              <div className="game-player-avatar">
                  ...
                  {gameStatus === 'in_progress' && (() => {
                    ...
                    const timeLimitSeconds = gameInfo?.moveTimeLimit ? Math.floor(gameInfo.moveTimeLimit / 1000) : 15
                    // Если таймер равен timeLimitSeconds или больше, показываем полный круг (progress = 1)
                    // Это гарантирует, что при начале игры таймер показывается полным
                    const progress = isOvertime 
                      ? Math.max(0, Math.min(1, opponentTotalTime / 60))
                      : opponentTimer >= timeLimitSeconds 
                        ? 1 
