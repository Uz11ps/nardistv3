...
echo "🔧 Прямое исправление конфигурации Nginx..."
...
if nginx -t 2>&1; then
    ...
    grep -n "location /" "$CONFIG_FILE" | grep -v "location /api" | grep -v "location /socket" | grep -v "location /health"
    echo ""
    echo "Показываем строки вокруг проблемного места:"
    PROBLEM_LINE=$(nginx -t 2>&1 | grep "duplicate location" | grep -oE '[0-9]+' | head -1)
    if [ ! -z "$PROBLEM_LINE" ]; then
        echo "Проблемная строка: $PROBLEM_LINE"
        sed -n "$((PROBLEM_LINE-5)),$((PROBLEM_LINE+5))p" "$CONFIG_FILE"
    fi
    echo ""
